...
NEXT_PUBLIC_INSTAGRAM_URL=https://www.instagram.com/clinicaarvorear/
```

- `NEXT_PUBLIC_SITE_URL` alimenta canonical, Open Graph, JSON-LD, sitemap e robots. Use a URL HTTPS sem barra no final.
- `NEXT_PUBLIC_INSTAGRAM_URL` deve apontar para o perfil oficial. Se estiver ausente ou inválida, o projeto usa `https://www.instagram.com/clinicaarvorear/` como fallback.
...
No deploy, configure as duas variáveis no ambiente de produção. `NEXT_PUBLIC_SITE_URL` é indispensável para remover o `noindex` e gerar canonical, sitemap, robots e JSON-LD com o domínio correto.
...
- `espaco/conceito-3d/`: renders de conceito, não usados na galeria do site.
...
4. Em **Settings > Environment Variables**, cadastre `NEXT_PUBLIC_SITE_URL` e `NEXT_PUBLIC_INSTAGRAM_URL` para Production, Preview e Development.
...
- [ ] Instagram oficial configurado como `https://www.instagram.com/clinicaarvorear/`
