      ...
      <Container>
        <div className="grid gap-12 border-b border-sand/10 pb-14 md:grid-cols-[1.25fr_0.75fr_1fr]">
          <div>
            ...
            <p className="mt-5 text-xs uppercase tracking-[0.18em] text-peach">Afeto • Ciência • Desenvolvimento</p>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-peach">Navegue</p>
            <nav className="mt-5 grid grid-cols-2 gap-x-5 gap-y-3 md:grid-cols-1" aria-label="Navegação do rodapé">
              {navigation.map((item) => <a key={item.href} href={item.href} className="w-fit text-sm text-sand/75 transition hover:text-sand">{item.label}</a>)}
            </nav>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-peach">Contato</p>
        ...
        <div className="flex flex-col gap-4 pt-7 text-xs text-sand/55 lg:flex-row lg:items-center lg:justify-between">
