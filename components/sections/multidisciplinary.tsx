      ...
      <Container>
        <Reveal className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-24">
          ...
          <div className="flex flex-col justify-end">
            ...
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex w-fit items-center gap-2 border-b border-peach/50 pb-2 text-[0.95rem] font-semibold text-sand/90 transition hover:border-peach hover:text-sand"><MessageCircle size={17} aria-hidden="true" /> Conversar com a equipe <ArrowUpRight size={16} aria-hidden="true" /></a>
          </div>
        </Reveal>
            ...
            return (
              <Reveal key={area.title} delay={index * 0.05} className="h-full">
                <article className="group flex h-full min-h-[270px] flex-col rounded-[1.8rem_1.8rem_1.8rem_0.6rem] border border-sand/10 bg-sand/[0.045] p-7 transition duration-300 hover:-translate-y-0.5 hover:border-serene/35 hover:bg-sand/[0.075] hover:shadow-[0_18px_46px_rgba(31,35,27,0.14)] motion-reduce:transform-none">
                  ...
                  <div className="mt-auto pt-10"><h3 className="font-display text-2xl text-sand">{area.title}</h3><p className="mt-3 text-[0.95rem] leading-7 text-sand/80">{area.text}</p></div>
                </article>
              </Reveal>
            );
          })}
